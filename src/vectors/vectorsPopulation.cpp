...
VectorsPopulation::VectorsPopulation(const string &name) :
	Atomic(name),
...
Model &VectorsPopulation::initFunction(){
	state = PASSIVE;
	passivate();
	return *this;
}
...
Model &VectorsPopulation::externalFunction(const ExternalMessage &msg){
...
Model &VectorsPopulation::internalFunction(const InternalMessage &msg){
	state = PASSIVE;
	passivate();
	return *this;
}
...
Model &VectorsPopulation::outputFunction(const CollectMessage &msg){
	switch (this->state) {
